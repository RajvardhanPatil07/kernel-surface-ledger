...
export function summarize(report: KslReport, hostLabel?: string): ScanSummary {
  ...
  return {
    ...
    collected_at: Number.isNaN(collected.getTime()) ? new Date().toISOString() : collected.toISOString(),
...
export function groundingContext(report: KslReport): string {
  ...
  lines.push('', 'SURFACE ELEMENTS (id | kind | subsystem | weight | present/reachable_unpriv/used | cves | gate)')
  ...
  for (const w of report.workloads) {
    lines.push(`${w.id} | ${w.comm} | ${w.unit ?? '-'} | uid=${w.uid ?? '-'} | ${(w.caps_effective ?? []).join(',') || '-'} | ${w.touches.join(',')}`)
  }
  lines.push('', 'LEDGER (workload_id | surface_debt | marginal | sole_owned | shared | reachable_cves)')
  for (const r of report.ledger) {
    lines.push(`${r.workload_id} | ${r.surface_debt} | ${r.marginal_contribution ?? '-'} | ${r.sole_owner_elements.join(',') || '-'} | ${r.shared_elements.join(',') || '-'} | ${r.reachable_cves}`)
  }
  lines.push('', `ORPHANED weight=${report.orphaned.total_weight} cves_neutralizable=${report.orphaned.cves_neutralizable} elements=${report.orphaned.elements.join(',')}`)
  ...
  for (const p of [...report.plan].sort((a, b) => a.step - b.step)) {
    lines.push(`${p.step} | ${p.action} | ${p.targets.join(',')} | ${p.cves_killed} | ${p.weight_removed ?? '-'} | ${p.breakage_risk} | ${p.breakage_note ?? '-'}`)
  }
  return lines.join('\n')
}
