...
import { elementIndex, workloadIndex } from '@/lib/ksl-report'
...
export function diffReports(base: KslReport, next: KslReport): ReportDiff {
  ...
  const commOf = (id: string): string => {
    const wl =
      workloadIndex(next).get(id) ??
      workloadIndex(base).get(id) ??
      undefined
    return wl?.comm ?? id
  }
  ...
  debtChanges.sort((a, b) => Math.abs(b.delta) - Math.abs(a.delta) || a.workloadId.localeCompare(b.workloadId))
